...
## Code actions
gd - go to defenition (change to <leader>cd)
gf - see all the stuff (change to <leader>ci)
...
<leader>ei - show info for error under the cursor
<leader>el - show line diagnostics
<leader>ek - go to previous error
<leader>ej - go to next error

## NeoVim (lazyvim) specifics
K - show hover documentation (types and etc.)
] or [ + b - move between buffers
